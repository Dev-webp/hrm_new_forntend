...
export default function LetterPage({ type, title }) {
  ...
  const selectEmployee = (employeeId) => {
    ...
    if (!employee) {
      return updateForm("employee_id", employeeId);
    }
    ...
    setForm((current) => ({
      ...
      joining_date: employee.joining_date
        ...
        : "",
      recipient_email:
        employee.email || current.recipient_email || "",
    }));
  };
  ...
  const download = () =>
    run(async () => {
      const response = await downloadLetter(type, form);
  ...
  return (
    <section className="letters-page">
      ...
      <p>
        Create, preview, generate, download, or email the current
        letter.
      </p>

      {notice && <p className="letter-notice">{notice}</p>}
      ...
      <LetterForm
        ...
        onGenerate={() =>
          run(async () => {
            const response = await generateLetter(type, form);
            setForm(response.data.request);
