...
function StructuredLogDetails({ log }) {
  ...
  const next = m.newValues || {};
  const type = log?.action_type;
  let fields = [];
  if (type === "attendance_changed") fields = [
    ["Employee", m.editedFor?.name], ["Changed by", m.editedBy?.name], ["Date", m.date],
    ["Old check-in", old.check_in_time], ["New check-in", next.check_in_time],
    ["Old check-out", old.check_out_time], ["New check-out", next.check_out_time], ["Reason", m.reason],
  ];
  if (type === "leave_changed") fields = [
  ...
  if (type === "employee_status_changed") fields = [
    ["Employee", m.changedEmployee || m.editedFor?.name], ["Old status", m.oldStatus || old.status],
    ["New status", m.newStatus || next.status], ["Changed by", m.changedBy || m.editedBy?.name], ["Reason", m.reason],
  ];
  if (!fields.length) return null;
  return (
    <div className="structured-log-details">
      <div className="diff-title">Change Details</div>
...
export default function AdminActivityLogs() {
  ...
  const handleDeleteLog = async (logId) => {
    if (!window.confirm("Delete this activity log permanently? This cannot be undone.")) return;
    try {
      await deleteActivityLog(logId);
      if (detailLog?.id === logId) setDetailModalOpen(false);
  ...
  const handleDeleteSelectedLogs = async () => {
    ...
    if (!ids.length || !window.confirm(`Permanently delete ${ids.length} selected activity log(s)?`)) return;
    setBulkDeleting(true);
    try {
      const result = await deleteSelectedActivityLogs(ids);
      await Promise.all([loadLogs(), loadStats()]);
      showToast(`${result.deletedCount || 0} activity log(s) deleted`, "success");
    } catch (err) {
      showToast(err.response?.data?.message || "Failed to delete selected logs", "error");
    } finally {
  ...
  const handleDeleteLogsByRange = async () => {
    ...
    try {
      ...
      showToast(err.response?.data?.message || "Failed to delete logs by range", "error");
    } finally {
      setBulkDeleting(false);
    }
  };
      ...
      <div className="advanced-filters">
        ...
        <div className="filter-group">
          ...
          >
            ...
            <option value="attendance_changed">Attendance Changed</option>
            <option value="leave_changed">Leave Changed</option>
            <option value="break_changed">Break Changed</option>
      ...
      {canDeleteLogs ? (
        <div className="activity-bulk-toolbar">
          <label className="bulk-select-all">
            <input type="checkbox" checked={allPageLogsSelected} onChange={toggleAllPageLogs} />
            Select page
          </label>
                ...
                logsData.map((log) => {
                  ...
                  return (
                    <tr key={fields.id} className={rowClass || undefined}>
                      <td className="select-column"><input type="checkbox" checked={selectedLogIds.has(fields.id)} onChange={() => toggleLog(fields.id)} disabled={!canDeleteThisLog} title={!canDeleteThisLog ? "System-wide logs are read-only for managers" : ""} aria-label={`Select log ${fields.id}`} /></td>
                      ...
                      <td>
                        <div className="row-actions">
                          ...
                          {canDeleteLogs && canDeleteThisLog ? (
                            <button
                              type="button"
        ...
        <div className="modal-box" onClick={(e) => e.stopPropagation()}>
          ...
          {detailLoading ? (
            ...
            <>
              ...
              <StructuredLogDetails log={detailLog} />
